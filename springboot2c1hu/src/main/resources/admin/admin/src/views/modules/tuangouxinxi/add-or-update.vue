<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="团购价" prop="tuangoujia">
          <el-input v-model="ruleForm.tuangoujia" 
              placeholder="团购价" clearable  :readonly="ro.tuangoujia"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="团购价" prop="tuangoujia">
              <el-input v-model="ruleForm.tuangoujia" 
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shuliang = true;
          ...
          if(o=='tuangoujia'){
            this.ruleForm.tuangoujia = obj[o];
	    this.ro.tuangoujia = true;
            continue;
          }
          if(o=='xiangqing'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springboot2c1hu/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tuangouxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tuangouxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
